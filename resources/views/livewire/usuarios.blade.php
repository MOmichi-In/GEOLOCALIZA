<div>
                    ...
                    @forelse($usuarios as $usuario)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-3">{{ $usuario->name }}</td>
                             <!-- CAMBIO: Se muestra la Cédula del usuario -->
                            ...
                            <td class="px-6 py-3 text-center">
                                <div class="flex justify-center gap-2">
                                    <button wire:click="editar({{ $usuario->id }})" class="text-gray-600 hover:text-gray-800 text-xl material-symbols-outlined rounded p-1 transition-colors duration-200">edit_note</button>
                                    <button wire:click="eliminar({{ $usuario->id }})" onclick="return confirm('¿Estás seguro?')" class="text-red-700 hover:text-red-900 text-xl material-symbols-outlined rounded p-1 transition-colors duration-200">delete</button>
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <!-- CAMBIO: Se ajusta el colspan a 5 -->
    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg max-w-2xl w-full p-6 relative">
            ...
            <h2 class="text-xl font-bold mb-4 text-center">{{ $editando ? 'Editar Usuario' : 'Crear Usuario' }}</h2>
            ...
            <form wire:submit.prevent="guardar" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="password" wire:model="password" class="w-full px-4 py-2 border rounded-lg" placeholder="{{ $editando ? 'Dejar en blanco para no cambiar' : '' }}">
                    ...
                    <div>
                        ...
                        <select wire:model.live="rol" class="w-full px-4 py-2 border rounded-lg">
                            ...
                            @foreach(\App\Models\User::$availableRoles as $rolOption)
                                <option value="{{ $rolOption }}">{{ str_replace('_', ' ', $rolOption) }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex justify-end gap-3 pt-4">
                    <button type="submit" class="bg-gray-500 hover:bg-gray-600 text-white px-5 py-2 rounded">{{ $editando ? 'Actualizar Usuario' : 'Guardar Usuario' }}</button>
                    <button type="button" wire:click="ocultarModal" class="bg-red-700 hover:bg-red-700 text-white px-5 py-2 rounded">Cancelar</button>
                </div>
            </form>
        </div>
    </div>
